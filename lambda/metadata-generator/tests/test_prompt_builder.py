...
def test_build_basic_prompt():
    ...
    file_info = FileInfo(
        bucket="test-bucket", key="test-file.txt", content="This is a test file content."
    )
...
def test_build_prompt_with_required_fields():
    ...
    file_info = FileInfo(bucket="test-bucket", key="document.md", content="Content here")
...
def test_build_prompt_with_enum():
    ...
    file_info = FileInfo(bucket="test-bucket", key="document.txt", content="Content")
...
def test_build_prompt_with_const():
    ...
    file_info = FileInfo(bucket="test-bucket", key="file.txt", content="Content")

    schema = {"type": "object", "properties": {"version": {"type": "string", "const": "1.0"}}}
...
def test_truncate_long_content():
    ...
    file_info = FileInfo(bucket="test-bucket", key="large-file.txt", content=long_content)
...
def test_build_prompt_with_multiple_field_types():
    ...
    file_info = FileInfo(
        bucket="test-bucket", key="complex-doc.md", content="Complex document content"
    )
    ...
    schema = {
        ...
        "properties": {
            ...
            "is_published": {"type": "boolean", "description": "Publication status"},
...
def test_build_prompt_file_name_extraction():
    ...
    file_info = FileInfo(bucket="test-bucket", key="path/to/nested/document.pdf", content="Content")
...
def test_build_prompt_with_nested_schema():
    ...
    schema = {
        "type": "object",
        "properties": {"metadata": {"type": "object", "description": "Nested metadata"}},
    }
...
def test_build_prompt_with_unicode_content():
    ...
    schema = {
        "type": "object",
        "properties": {"title": {"type": "string", "description": "ドキュメントのタイトル"}},
    }
